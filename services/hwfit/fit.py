...
def rank_models(system, use_case=None, limit=50, search=None, sort="score", quant=None):
    ...
    for m in models:
        ...
        # MLX-quantized models only run via the MLX runtime (mlx_lm) on Apple
        # Silicon. Show them on Metal so users can download and run them with
        # mlx_lm directly; hide on every other backend (CUDA / ROCm / CPU)
        # since they're unrunnable there.
